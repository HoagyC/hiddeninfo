...
def handle_plot_kwargs(subplot=None, **kw) -> None:
    ...
    if kw.get("xlabel"):
        curr_plot.xlabel(kw["xlabel"])
    if kw.get("ylabel"):
        curr_plot.ylabel(kw["ylabel"])
    if kw.get("margins"):
        curr_plot.margins(kw["margins"])
    if kw.get("xticks"):
        curr_plot.xticks(**kw["xticks"])
    if kw.get("yticks"):
        curr_plot.yticks(**kw["yticks"])
    if kw.get("xlim"):
        curr_plot.xlim(**kw["xlim"])
    if kw.get("ylim"):
        curr_plot.ylim(**kw["ylim"])
    if kw.get("set_xlim"):
        curr_plot.set_xlim(**kw["set_xlim"])
    if kw.get("set_ylim"):
        curr_plot.set_ylim(**kw["set_ylim"])
    if kw.get("subplots_adjust"):
...
def convert_continuous_back_to_ordinal(y_true, y_pred, use_integer_bins=False):
    ...
    y_pred_binned_i = assign_value_to_bins(
        y_pred, unique_y_true, use_integer_bins=use_integer_bins
    )
...
def assess_performance(y, yhat, names, prediction_type, prefix, verbose=False):
    ...
    """
    assert y.shape == yhat.shape, print(
        "(%s) y: %s, yhat: %s" % (prefix, str(y.shape), str(yhat.shape))
    )
    assert y.shape[1] == len(names), print(
        "%s) y: %s, len(names): %d" % (prefix, str(y.shape), len(names))
    )
    ...
    for i, name in enumerate(names):
        ...
        if prediction_type == "binary":
            ...
            if prediction_type == "continuous_ordinal":
                yhat_round_i, cat_y_i = convert_continuous_back_to_ordinal(
                    y_i, yhat_i, use_integer_bins=True
                )
                precision, recall, fbeta, support = precision_recall_fscore_support(
                    cat_y_i, yhat_round_i
                )
        ...
        if verbose:
            if prediction_type in ["multiclass", "continuous_ordinal"]:
                ...
                out = ("%11s |" % prefix_name[:-1]) + ("%8s|" * N_classes) % tuple(
                ...
                for metric in ["precision", "recall", "F1", "support"]:
                    ...
                    for cls_id in range(N_classes):
                        if metric == "support":
                            ...
                            out += "  %04.1f  |" % (
                                metrics[prefix_name + metric][cls_id] * 100.0
                            )
                out += "\nMacro precision: %2.1f" % (
                    (metrics[prefix_name + "macro_precision"]) * 100.0
                )
                out += "\nMacro recall   : %2.1f" % (
                    (metrics[prefix_name + "macro_recall"]) * 100.0
                )
                out += "\nMacro F1       : %2.1f" % (
                    (metrics[prefix_name + "macro_F1"]) * 100.0
                )
                print(out)
