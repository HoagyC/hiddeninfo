...
class CLIP_CUB_Dataset(Dataset):
    def __init__(self, data: List[CLIP_CUB_Entry]):
        self.data = data

    def __len__(self):
        return len(self.data)

    def __getitem__(self, ndx: int) -> Tuple[torch.Tensor, int]:
        entry = self.data[ndx]
        return entry.CLIP_embedding, entry.entry_class


def _make_data_dict() -> List:
    ...
    with open(CUB_PATH / "train_test_split.txt", "r") as f:
        for line in f:
            idx, is_train_str = line.strip().split()
            is_train_test[int(idx)] = int(is_train_str)

    id_to_class_dict: Dict = dict()
